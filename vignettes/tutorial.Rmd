...
In this the example the files `temperature_surface-water_TEGsee-mikrosieb.csv` 
and `temperature_groundwater_TEG343.csv` are used for testing the functionality
...
The sinus fit can not only be written as `y = y0 + amplitude * sin(x + phase)`, but also
...
phase (`x0`) by using R's build-in `stats:lm()` function as examplarly shown below:
...
day_number <-
  seq_len(period_length) # daily sequence from 1 to period_length
temperature <-
  26.9188 + 2.123641 * sin(2 * pi * day_number / period_length + 0.6049163)

### Print first data points:
tibble::tibble(day_number = day_number, temperature = temperature)


### Now check the if both formulas yield equal results

fit.lm2 <-
  lm(
    temperature ~ sin(2 * pi * day_number / period_length) + cos(2 * pi * day_number /
                                                                   period_length)
  )
...
accuracy of  10^`r.Machine$double.ulp.digits * log10(2)`,i.e.  10^`r.Machine$double.ulp.digits * log10(2)`
