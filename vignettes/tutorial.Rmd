...
The sinus fit can not only be written as `y0 + amplitude * sin(x + phase)`, but also
in a linear form `y0 + alpha * sin(x) + beta * cos(x)`. The latter approach is used 
...
### print show first data points:
tibble::tibble(day_number = day_number, temperature = temperature)


### Now check the if both formulas yield equal results

fit.lm2 <- lm(temperature ~ sin(2*pi*day_number/period_length) + cos(2*pi*day_number/period_length))
...
### Formula option 1: y0 + amplitude * sin(x + phase)
plot(x = day_number, 
     y = y0 + a* sin(x + x0),
     type = "l",
     lty = 1,
...
### Formula option 2: y0 + alpha * sin(x) + beta * cos(x)
