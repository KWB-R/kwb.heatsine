...
run_montecarlo <- function(sinus_fit_list, nMC) {
  ...
  sinus_prediction <- as.data.frame(predict(object = sinus_fit_list$lm_object,
                                            interval = "confidence"))
  
  sd_df <- tibble::tibble(sd_y0 = 0, 
                          sd_a = 0, 
                          sd_x0 = 0, 
                          sd_p = 0, 
...
paras <- sinus_fit_list$paras %>%  
 ...
 res_montecarlo_list <- lapply(seq_len(nMC),function(run_id) {
    # generate random parameter values
    number_of_days <- length(sinus_fit_list$lm_object$model$value)
    x <- seq_len(number_of_days)
    y0_r <- rnorm(1, mean = paras$y0, sd = paras$sd_y0)
    a_r  <- rnorm(1, mean = paras$a, sd = paras$sd_a)
    x0_r <- rnorm(1, mean = paras$x0, sd = paras$sd_x0)
    p_r  <- rnorm(1, mean = paras$period_length, sd = paras$sd_p)
    ...
    # positionmini <- order(y_r)[1:1]
    
    # get position (year) of extreme value maximum
    #positionmini <- order(y_r, decreasing = TRUE)[1:1]
 ...
 dplyr::bind_rows(res_montecarlo_list)
} 
